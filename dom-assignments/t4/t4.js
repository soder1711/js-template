...
function success(position) {
  ...
  for (let restaurant of restaurants) {
    let longitude = restaurant.location.coordinates[0] - myLongitude;
    let latitude = restaurant.location.coordinates[1] - myLatitude;
    restaurant.distance = Math.sqrt(Math.pow(longitude, 2) + Math.pow(latitude, 2));
  }
  restaurants.sort((a, b) => a.distance - b.distance);
}
const table = document.querySelector("table");
for (let restaurant of restaurants) {
  table.insertAdjacentHTML("beforeend", `<tr><th>${restaurant.name}</th><th>${restaurant.address}</th></tr>`)
}
